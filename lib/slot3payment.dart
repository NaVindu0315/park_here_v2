...
class slot3pay extends StatefulWidget {
  const slot3pay({Key? key}) : super(key: key);

  @override
  _slot3payState createState() => _slot3payState();
}

class _slot3payState extends State<slot3pay> {
  ...
  void initState() {
    super.initState();
    _databaseReference =
        FirebaseDatabase.instance.reference().child('Vehicle2/ParkingDuration');
    ...
    _databaseReference =
        FirebaseDatabase.instance.reference().child('Vehicle2/ParkingCost');
    _databaseReference.onValue.listen((event) {
      setState(() {
  ...
  void update(double value) {
    _database.child('Vehicle2/ParkingCost').set(value);
  }

  ///cost end
  ///time
  void uptime(String value) {
    _database.child('Vehicle2/ParkingDuration').set(value);
  }
